...
print()
print("=" * 60)
print("8) LEVEL TRACKER (XP chat + leveling)")
print("=" * 60)
import importlib.util as _ilu2

_lspec = _ilu2.spec_from_file_location("levels", os.path.join(REPO_DIR, "levels.py"))
lvlmod = _ilu2.module_from_spec(_lspec)
_lspec.loader.exec_module(lvlmod)
lvl = lvlmod.LevelTracker(os.path.join(tempfile.mkdtemp(), "lvl.json"))
check("xp_needed 1->2 = 100", lvl.xp_needed(1) == 100)
check("xp_needed 2->3 = 250", lvl.xp_needed(2) == 250)
check("xp_needed 3->4 = 500", lvl.xp_needed(3) == 500)
rec, leveled = lvl.add_xp("u1", 100)
check("100 XP -> level 2", rec["level"] == 2 and leveled)
rec, leveled = lvl.add_xp("u1", 250)
check("+250 XP -> level 3", rec["level"] == 3 and leveled)
rec, leveled = lvl.add_xp("u1", 500)
check("+500 XP -> level 4", rec["level"] == 4 and leveled)
rec, leveled = lvl.add_xp("u1", 50)
check("+50 XP belum naik (level 4, xp 50)", rec["level"] == 4 and rec["xp"] == 50 and not leveled)
check("progress() bener", lvl.progress("u1") == (4, 50, 850))
lvl.add_xp("u2", 300)
lb = lvl.leaderboard(10)
check("leaderboard urut level", lb[0][0] == "u1" and lb[1][0] == "u2", str(lb))
lvl2 = lvlmod.LevelTracker(lvl.state_file)
check("state persist ke disk", lvl2.progress("u1") == (4, 50, 850))
